...
const LinkItem = ({ template, editPath }) => {
  if (template.id && template.canEdit) {
    const key =
      template.className.charAt(0).toLowerCase() + template.className.slice(1);
    const editUrl = editPath[key]?.replace(':id', template.id);

    return (
      <a
        href={editUrl.replace(':id', template.id)}
        target="_blank"
...
const extractAttr = (template, attr, classNameMap) =>
  classNameMap[template[attr]] ? classNameMap[template[attr]] : template[attr];
